...
'''
from generative_compression.network import Network
from generative_compression.data import Data
from generative_compression.config import directories
from generative_compression.utils import Utils
'''
...
class Model():
    def __init__(self, config, paths, dataset, name='gan_compression', evaluate=False):
        ...
        self.train_writer = tf.summary.FileWriter(
            os.path.join(directories.tensorboard, '{}_train_{}'.format(name, time.strftime('%d_%m_%I_%M'))), graph=tf.get_default_graph())
           #os.path.join(directories.tensorboard, '{}_train_{}'.format(name, time.strftime('%d-%m_%I:%M'))), graph=tf.get_default_graph())
        self.test_writer = tf.summary.FileWriter(
            os.path.join(directories.tensorboard, '{}_test_{}'.format(name, time.strftime('%d-%m_%I_%M'))))
